...
export function useTypingEngine() {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.key === "Shift" || e.key === "CapsLock" || e.key === "Escape" || e.key === "Enter" || e.key === "Tab") return;
      if (s.words.length === 0) return;

      const isZen = useConfigStore.getState().mode === "zen";
      ...
      if (isZen) {
        const letter: Letter = { char: e.key, status: "correct" };
        const letters = [...word.letters, letter];
        const newTyped = word.typed + e.key;
        s.updateWord(s.currentWordIndex, { ...word, letters, typed: newTyped });
        s.setCurrentLetterIndex(s.currentLetterIndex + 1);
      } else {
  ...
  return { handleKeyDown, finishTest, clearTimers };
}
